...
import { LayoutDashboard, Users, Package, ShoppingCart, TrendingUp, Bell, Settings, Menu, X, Store } from 'lucide-react';
    ...
    <div className="min-h-screen flex">
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:bg-white lg:border-r lg:border-gray-200">
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Karobar</h1>
              <p className="text-xs text-gray-500">{user?.shopName || 'My Shop'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-100">
          ...
          <button onClick={logout} className="w-full text-left text-sm text-gray-500 hover:text-red-600 transition-colors px-1">
            Log out
          </button>
        </div>
      </aside>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-900">Karobar</span>
        </div>
          ...
          <div className="fixed inset-y-0 left-0 w-72 bg-white shadow-xl p-4" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-bold text-gray-900">Karobar</h1>
            ...
            <div className="absolute bottom-4 left-4 right-4">
              <button onClick={logout} className="w-full text-left text-sm text-gray-500 hover:text-red-600 px-3 py-2">
                Log out
              </button>
            </div>
          </div>
        ...
        <div className="flex justify-around items-center h-16 px-2">
          {navItems.slice(0, 5).map(item => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium transition-colors ${
                location.pathname === item.path
                  ? 'text-primary-600'
                  : 'text-gray-500'
              }`}
          ...
          <Link
            to="/notifications"
            className={`flex flex-col items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium transition-colors ${
              location.pathname === '/notifications' || location.pathname === '/settings'
                ? 'text-primary-600'
                : 'text-gray-500'
            }`}
          >
