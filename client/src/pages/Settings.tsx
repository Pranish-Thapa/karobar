...
export default function Settings() {
  ...
  const handleSaveShop = async (e: React.FormEvent) => {
    ...
    try { await api.settings.updateShop(shopName); updateUser({ shopName }); setMessage(t.success + '!'); } catch (err: any) { setMessage(err.message); } finally { setSaving(false); }
  };

  const generateQR = async () => {
    setQrLoading(true);
    try { const res = await api.qr.generate(); setQrImage(res.qr); setQrExpiry(res.expiresAt); } catch (err: any) { alert(err.message); } finally { setQrLoading(false); }
  };
    ...
    <div className="pb-20 lg:pb-0 max-w-2xl mx-auto">
      ...
      <div className="card mb-6">
        ...
        <form onSubmit={handleSaveShop} className="space-y-4">
          ...
          <button type="submit" disabled={saving} className="btn-primary">{saving ? '...' : t.saveChanges}</button>
        </form>
      </div>
      ...
      <div className="card mb-6">
        ...
        <div className="space-y-3">
          ...
          <button onClick={() => setLanguage('ne')} className={`w-full flex items-center justify-between p-3 rounded-lg border transition-colors ${lang === 'ne' ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
            <span className="text-gray-900 dark:text-white font-medium">नेपाली</span>
        ...
        <div className="text-center mb-4">
          {qrImage ? (
            <div>
              ...
              <button onClick={generateQR} className="btn-secondary mt-3 text-sm flex items-center gap-2 mx-auto"><RefreshCw className="w-3 h-3" /> {t.generateNewQR}</button>
            </div>
          ) : (
      ...
      <div className="card">
        ...
        : <div className="space-y-3">{devices.map(device => (
          <div key={device.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            ...
            <button onClick={() => disconnectDevice(device.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"><Trash2 className="w-4 h-4" /></button>
          </div>
        ))}</div>}
      </div>
    </div>
