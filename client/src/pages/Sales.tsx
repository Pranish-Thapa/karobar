...
function formatDate(dateStr: string) { return new Date(dateStr).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }); }
...
export default function Sales() {
  ...
  const loadData = () => {
    Promise.all([api.sales.list({ search, from: dateFrom || undefined, to: dateTo || undefined }), api.orders.list(), api.customers.list(), api.products.list()]).then(([s, o, c, p]) => { setSales(s); setOrders(o); setCustomers(c); setProducts(p); }).finally(() => setLoading(false));
  };
  useEffect(() => { loadData(); }, [search, dateFrom, dateTo]);
  ...
  const handleNewSale = async (e: React.FormEvent) => {
    ...
    try { const result = await api.sales.create({ customer_id: saleCustomerId || undefined, items: saleItems.filter(i => i.product_id), amount_paid: parseFloat(salePaid) || 0 }); setSaleSuccess(result); setShowNewSale(false); setSaleCustomerId(''); setSaleItems([]); setSalePaid(''); loadData(); } catch (err: any) { setSaleError(err.message); }
  };
  ...
  const handleNewOrder = async (e: React.FormEvent) => {
    ...
    try { await api.orders.create({ customer_id: orderCustomerId || undefined, items: orderItems.filter(i => i.product_id), amount_paid: parseFloat(orderPaid) || 0, expected_date: orderExpectedDate || undefined, notes: orderNotes || undefined }); setShowNewOrder(false); setOrderCustomerId(''); setOrderItems([]); setOrderPaid(''); setOrderExpectedDate(''); setOrderNotes(''); loadData(); } catch (err: any) { setOrderError(err.message); }
  };

  const completeOrder = async (orderId: string) => { try { await api.orders.complete(orderId); loadData(); } catch (err: any) { alert(err.message); } };

  const handleReturn = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await api.returns.create({ sale_id: returnSaleId, product_id: returnProductId, quantity: parseInt(returnQty), reason: returnReason }); setShowReturn(false); setReturnSaleId(''); setReturnProductId(''); setReturnQty(''); setReturnReason(''); loadData(); } catch (err: any) { alert(err.message); }
  };
  ...
  return (
    <div className="pb-20 lg:pb-0">
      ...
      <div className="mb-4">
        <div className="flex gap-2">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" /><input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t.search + '...'} className="input pl-10" /></div>
      ...
      <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1 mb-6">
        <button onClick={() => setTab('sales')} className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${tab === 'sales' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400'}`}>{t.completedSales} ({sales.length})</button>
      ...
      : tab === 'sales' ? (
        ...
        : <div className="space-y-3">{upcomingOrders.map(order => (<div key={order.id} className="card"><div className="flex items-start justify-between mb-2"><div><p className="font-semibold text-gray-900 dark:text-white">{order.customer_name || t.walkInCustomer}</p><p className="text-sm text-gray-500">{t.from}: {formatDate(order.order_date)}</p>{order.expected_date && <p className="text-sm text-gray-500">{t.expectedDate}: {formatDate(order.expected_date)}</p>}</div><p className="font-bold text-gray-900 dark:text-white">{formatCurrency(order.total_amount)}</p></div><p className="text-sm text-gray-600 dark:text-gray-300 mb-3">{order.items || '-'}</p><button onClick={() => completeOrder(order.id)} className="btn-primary text-sm w-full sm:w-auto"><CheckCircle className="w-4 h-4 inline mr-1" /> {t.markCompleted}</button></div>))}</div>
      )}
      ...
      {showNewSale && <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleNewSale} className="p-4 space-y-4">
          ...
          <div className="flex gap-3 pt-2"><button type="button" onClick={() => setShowNewSale(false)} className="btn-secondary flex-1">{t.cancel}</button><button type="submit" className="btn-primary flex-1">{t.completeSale}</button></div>
        </form>
      </div></div>}
      ...
      {showNewOrder && <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleNewOrder} className="p-4 space-y-4">
          ...
          <div className="flex gap-3 pt-2"><button type="button" onClick={() => setShowNewOrder(false)} className="btn-secondary flex-1">{t.cancel}</button><button type="submit" className="btn-primary flex-1">{t.createOrder}</button></div>
        </form>
      </div></div>}
      ...
      {showReturn && <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4"><div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md">
        ...
        <form onSubmit={handleReturn} className="p-4 space-y-4">
          ...
          <div className="flex gap-3"><button type="button" onClick={() => setShowReturn(false)} className="btn-secondary flex-1">{t.cancel}</button><button type="submit" className="btn-primary flex-1">{t.confirm}</button></div>
        </form>
      </div></div>}
    </div>
  );
